...
//Удалить из него К элементов, начиная с заданного номера, добавить К элементов, начиная с заданного номера;
...
void print(point* first) // вывод списка
{
    if (first == NULL)
...
point* add_elements(point* first, int number, int k) // добавить элементы
{
    if (first == NULL)
    {
        return first;
...
void free_list(point** first) // удалить список
{
    point* p = *first, * t;
    while (p != NULL)
    {
...
void save_to_file(point* first, const char* filename) // сохранить в файл
{
    if (first == NULL)
    {
...
point* read_file(const char* filename) // прочитать файл
{
